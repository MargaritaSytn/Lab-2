...
// Визначаємо ідентифікатори ресурсів прямо тут, щоб уникнути помилок з include
...
int main() {
    ...
    int choice;
    std::cout << "========================================" << std::endl;
    std::cout << "Оберiть мову написавши цифру:" << std::endl;
    std::cout << "1 - Українська" << std::endl;
    ...
    if (choice == 1) {
        hLangLib = LoadLibrary(TEXT("LangUKRdll.dll"));
    }
    else if (choice == 2) {
        hLangLib = LoadLibrary(TEXT("LangENGdll.dll"));
    }
    else {
    ...
    if (hLangLib == NULL) {
        ...
        std::cerr << "Переконайтеся, що LangUKRdll.dll та LangENGdll.dll лежать у папцi з EXE-файлом." << std::endl;
        return 1;
    }
